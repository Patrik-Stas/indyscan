const txTypes = {
    ...
    102: { name: 'CLAIM_DEF', description: "Adds a claim definition (in particular, public key), that Issuer creates and publishes for a particular Claim Schema" },
    109: { name: 'POOL_UPGRADE', description: "Command to upgrade the Pool (sent by Trustee). It upgrades the specified Nodes (either all nodes in the Pool, or some specific ones).    " },
...
export function describeTransaction(tx) {
    ...
    const type = txCodeToTxType(code)
    switch (typeCode) {
        case "NODE":
