...
resource "null_resource" "provision-indypool" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "\"$SCRIPT_DIR_PATH\"/indypool/start.sh --address ${aws_instance.indyscan.public_ip} --version v1.8.3 --name '${var.network-name}'",
      "\"$SCRIPT_DIR_PATH\"/indyscan-daemon/start.sh --mode build --url-mongo mongodb://localhost:27999 --indy-networks '${var.network-name}'",
      "\"$SCRIPT_DIR_PATH\"/indyscan-webapp/start.sh --mode build --url-mongo mongodb://mongo.indynet:27017 --indy-networks '${var.network-name}'"
    ]
  }
}
