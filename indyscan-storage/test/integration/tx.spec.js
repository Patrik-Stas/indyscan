/* eslint-env jest */
import { createLedgerStorageManager } from '../../src/index'
import {
  andFilters,
...
describe('basic storage test', () => {
  ...
  it('should by default get all transactions sorted with newest(idx=0) to oldest', async () => {
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs()
  ...
  it('should get range of transactions', async () => {
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 10)
    expect(Array.isArray(txs)).toBeTruthy()
  ...
  it('should get transactions of specific type', async () => {
    ...
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 10, txFilter)
    for (const tx of txs) {
      expect(tx.rootHash).toBeDefined()
      expect(tx.auditPath).toBeDefined()
      expect(tx.txnMetadata).toBeDefined()
  ...
  it('should get transactions with txn before specific time', async () => {
    ...
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 12, txFilter)
    expect(Array.isArray(txs)).toBeTruthy()
  ...
  it('should get transactions with txn after specific time', async () => {
    ...
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 300, txFilter)
  ...
  it('should bound transaction time using 2 txntime filters', async () => {
    ...
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 300, boundTimerangeFilter)
    console.log(txs.length)
    expect(Array.isArray(txs)).toBeTruthy()
  ...
  it('should combine 3 filters', async () => {
    ...
    const txs = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 300, combinedFilter)
    expect(areTxsAfterTime(txs, 1519920217)).toBeTruthy()
    expect(areTxsBeforeTime(txs, 1520365010)).toBeTruthy()
  ...
  it('should return timestamp of oldest transaction which has a timestamp', async () => {
    const oldest = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getOldestTimestamp()
  ...
  it('should get transaction in seqNo range', async () => {
    ...
    const timestamps = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxs(0, 100, filter)
  ...
  it('should sort by default get all timestamps sorted from newest (idx=0) to oldest', async () => {
    const timestamps = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxsTimestamps()
  ...
  it('should sort txs from newest to oldest by default', async () => {
    const timestamps = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxsTimestamps(0, 50)
  ...
  it('should get less than 100 timestamps if filter is applied', async () => {
    ...
    const timestamps = await ledgerManager.getLedger(INDY_NETWORK, 'domain').getTxsTimestamps(0, 100, filter)
