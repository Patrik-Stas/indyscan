...
describe('basic api test suite', () => {
  it('should get all workers', async () => {
    const workers = await daemonClient.getWorkers(process.env.API_URL)
  ...
  it('should get specific workers by ids', async () => {
    ...
    const workersFiltered = await daemonClient.getWorkers(process.env.API_URL, daemonQueryBuilder.buildWorkersQuery(undefined, undefined, undefined, [workerId0, workerId2]))
  ...
  it('should get workers by operation type', async () => {
    const workers = await daemonClient.getWorkers(process.env.API_URL, daemonQueryBuilder.buildWorkersQuery('expansion'))
    expect(workers.length).toBeGreaterThan(0)
  ...
  it('should get workers by subledger', async () => {
    const workers = await daemonClient.getWorkers(process.env.API_URL, daemonQueryBuilder.buildWorkersQuery(undefined, 'domain', undefined, undefined))
    expect(workers.length).toBeGreaterThan(0)
  ...
  it('should get workers by target es index', async () => {
    const workers = await daemonClient.getWorkers(process.env.API_URL, daemonQueryBuilder.buildWorkersQuery(undefined, undefined, 'txs-sovmain'))
    expect(workers.length).toBeGreaterThan(0)
