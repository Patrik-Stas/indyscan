...
describe('basic iterator testsuite', () => {
  it('should return the same data if getNextTx called repeatedly without modifying guided storage state', async () => {
    let iterator = createIteratorGuided({ id: 'test-iterator', source: txSource1, sourceSeqNoGuidance: txSource2, guidanceFormat: 'format-foo' })
  ...
  it('should return undefined if no next transaction available', async () => {
    let iterator = createIteratorGuided({ id: 'test-iterator', source: txSource1, sourceSeqNoGuidance: txSource2, guidanceFormat: 'format-pool' })
    let { tx: tx1, meta: meta1 } = await iterator.getNextTx('pool', 'format-pool')
